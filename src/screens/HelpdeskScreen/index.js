...
export default class ExperimenScreen extends React.Component {
  ...
  validationForm() {
    if (this.state.ticketCatIdSend != 0 ){
      ...
      return this.registerCallToReopen();
    }
    }else{
      if (this.state.dataTitle === '') {
      return this.setState({ isModalFormKosongTitle: true, isLoadingSubmit: false });
  ...
  actionShow1() {
    this.setState({ show1: true, show3: false })
  }
  actionShow1false() {
    this.setState({ show1: false })
  }
  actionShow3() {
    this.setState({ show3: true, show1: false })
  }
  actionShow3false() {
  ...
  actionHelpdesk() {
    const resetAction = StackActions.reset({
      ...
      actions: [
        NavigationActions.navigate({
          routeName: 'HelpdeskScreens', params: {
            ticketCatIdSend: 0,
            trNoSend: ''
          } })
      ]
  ...
  render() {
    let viewShow1, viewShow3, showClose, showMenu, showRef;
