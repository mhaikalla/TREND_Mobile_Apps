...
import { CardItem, Textarea, Form, Item, Label, Input, Container, Header, Content, Button, Icon, Picker } from 'native-base';
...
export default class ExperimenScreen extends React.Component {
  ...
  getListPartipant() {
    fetch(API.getListParticipant, {
      ...
      .then((response) => response.json())
      .then((responseJson) => {
        // console.log('ini participant '+ JSON.stringify(responseJson))
    ...
    fetch(API.registerProduct,
      {
       ...
       console.log("Error Convert Json = "+eror);
      })
      .then((response) => {
  ...
  saveDraft() {
    ...
    // Alert.alert("Body sent to API", JSON.stringify(formData))
    // console.log('ini akan di send ke API: ' + JSON.stringify(formData))
    fetch(API.registerProduct,
      {
        'body': formData,
  ...
  saveDraftReopen() {
    ...
    // Alert.alert("Reopen Save Draft Body", JSON.stringify(formData))
    fetch(API.registerProduct,
      {
        'body': formData,
        headers: {
    ...
    ImagePicker.showImagePicker(options, (response) => {
      console.log("RESPONSE :", response)
        if(response.uri){
          ...
          // this.compressImage(response.uri, response.originalRotation)
        }
  ...
  //   var index = this.state.multipleFile.findIndex(item => item.uri === items)
  //   this.state.multipleFile.splice(index, 1)
  //   setTimeout(() => {
        ...
        <View style={{ backgroundColor: 'white' }}>
          <View style={{ marginTop: 20, marginLeft: 20, marginRight: 20 }}>
            <View style={{ flexDirection: 'row', width: '100%' }}>
              ...
              <View style={{ width: '75%' }}>
                <Text style={styles.textall}>Product Health</Text>
              </View>
            </View>
          </View>
          <View style={{ marginTop: 20, marginLeft: 20, marginRight: 20 }}>
        ...
        <View style={{ padding: 10 }}>
          <View style={{ borderRadius: 5, borderWidth: 1, padding: 10, borderColor: '#D7E0F1' }}>
            ...
            <TouchableOpacity onPress={this.selectMultipleFile.bind(this)}>
              <View style={{ width: '40%', flexDirection: 'row', justifyContent: 'space-between', backgroundColor: '#CCCCCC', padding: 10, borderRadius: 10 }}>
                <Text style={{ marginTop: 5 }}>Browse</Text>
                  ...
                  return (
                    <View style={{ flexDirection: 'column', marginLeft: 10, marginRight: 15, padding: 15, }}>
                      ...
                      <TouchableOpacity onPress={() => this.actionDeleteFile(id)}>
                        <View style={{ backgroundColor: '#CCCCCC', padding: 8, borderRadius: 100, width:40 }}>
